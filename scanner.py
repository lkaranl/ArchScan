...
UDP_sig 	= {
			"archscan"	: b"\x61\x72\x63\x68\x73\x63\x61\x6e",
...
def udp_scan(target_port):
    ...
    try:
        ...
        if (port == 123): 
            ...
            conn.sendto(UDP_sig["archscan"], (target,port))
...
class Dados(RawConfigParser):
    ...
    def Pegar_Dados(self):
        self.Verifica_Dados()
        self.read(Arq)
        IP = self.get("Dados","IP_")
        Protocolo = self.get("Dados","PRO")
        Portas = self.get("Dados","POR")
        return IP,Protocolo,Portas

    def Gravar_Dados(self):
        self.Verifica_Dados() 
        self.add_section("Porta")
        self.set("Porta","Open",str(ports_ident['open'])) 
        self.set("Porta","Close",str(ports_ident['closed'])) 
        self.set("Porta","Filte",str(ports_ident['filtered']))
        self.set("Porta","Opfil",str(ports_ident['open|filtered']))
        with open(Arq, "a") as FLconf:
            self.write(FLconf)
...
if __name__ == "__main__":
    ...
    print("[ %s ] Scan iniciano.\n" % (datetime.datetime.now().strftime("%H:%M:%S - %d/%m/%Y")))
    inicio = time.time()
   
    Dad = Dados()
    IP,Protocolo,Portas = Dad.Pegar_Dados()

    if Protocolo == 'UDP':
        SCAN_TYPE = "UDP"

    ports = parse_ports(Portas)
    target = IP
    q = Queue.Queue() 
