...
def Main_Window():
    ...
    def Scan():
        ...
        if Protocolo != 'UDP' and Protocolo != 'TCP':
            ...
            # Ajuste para lidar com a representação literal de listas no Python 3
            vopen = eval(OPEN) if OPEN.startswith('[') else OPEN.split(', ')
            vclose = eval(CLOSE) if CLOSE.startswith('[') else CLOSE.split(', ')
            vfilte = eval(FILTER) if FILTER.startswith('[') else FILTER.split(', ')
            vopfil = eval(OPFIL) if OPFIL.startswith('[') else OPFIL.split(', ')

            for op in range(0, len(vopen)):
                LIS_Open.insert(END, str(vopen[op]))

            for cl in range(0, len(vclose)):
                LIS_Close.insert(END, str(vclose[cl]))
                
            for fi in range(0, len(vfilte)):
                LIS_Filtrada.insert(END, str(vfilte[fi]))
            
            for of in range(0, len(vopfil)):
                LIS_OPFIL.insert(END, str(vopfil[of]))
